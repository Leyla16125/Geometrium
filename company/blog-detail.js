...
if (blog) {
  heroSection.style.backgroundImage = `url(${blog.image})`;
}
...
window.addEventListener("scroll", function () {
  if (window.scrollY > 50) {
    header.classList.add("bg-[#ddd]", "shadow-sm");
    header.classList.remove("text-white");
  } else {
    header.classList.remove("bg-[#ddd]", "shadow-sm");
  }
});
...
const exampleblog = document.getElementById("exampleblog");
const example = blogs.slice(0, 3);

example.forEach(blog => {
    ...
    exampleblog.appendChild(blogCard);
});
