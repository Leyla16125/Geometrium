...
const reviews = [
  {
    ...
    text: "Когда мы обратились в студию, хотели получить современный функциональный интерьер, в котором была бы продумана вся эргономика. Хотели также хорошую кухню, потому что мы оба любим готовить. Чтобы кухня закрывалась и открывалась, убирая лишний шум. Хотели общую зону, чтобы собираться с друзьями. Одновременно можно было и готовить, и общаться всем вместе. Вот это всё реализовано в нашей квартире. Мы очень довольны конечным продуктом. Следующий ремонт мы будем также сами реализовывать, но не без профессиональной помощи. Профессиональную помощь ничто не заменит. Здесь нужны определённые знания, чтобы досконально всё продумать и сделать.",
  ...
  },
  {
    ...
    text: "Всем остались довольны. Сделали проект нашей квартиры с архитектурным бюро GEOMETRIUM. Работой команды дизайнеров остались очень довольны! Всегда на связи (даже в выходные), всегда готовы выслушать и ненавязчиво внести свои коррективы. В результате нашей совместной работы мы получили очень интересный проект. Спасибо, успехов и процветания вашему креативному бюро!",
    subtitle: "Лилия",
    info: "Москва, 113 м²",
  ...
  },
  {
    ...
    text: "Для своей квартиры мы сделали целый тендер, чтобы выбрать студию дизайна. Анализировали на предмет качества, подхода, стоимости и стиля. У нас было несколько вариантов, которые нам подходили по дизайну и цене. Мы смотрим по качеству реализации интерьеров, много роликов на YouTube посмотрели, и в результате компания Geometrium выиграла тендер. Так как мы сами занимались реализацией интерьера, мы очень много времени уделили именно проектированию, чтобы всё было продумано, понятно и доступно для нас и наших строителей. Поэтому мы всё детально прорабатывали с дизайнером. Для нас это был очень крутой этап, создавать и реализовывать проект. Спасибо вашей студии!",
  ...
  },
  {
    ...
    text: "Хочется сказать спасибо дизайнеру Эльвире и в целом команде Geometrium, что им удалось в наши 85 кв.м уместить всё, что нам хотелось! Мы, конечно, долго утверждали планировочное решение, но в результате всё получилось. Три полноценные комнаты, два санузла, один с душем, один с ванной. Общая зона достаточно большая. Мы собирались там компанией в 15 человек, и было классно.",
    subtitle: "ЖК Небо",
    info: "95 м²",
  ...
  },
  {
    ...
    text: "Дорогая команда Geometrium, очень рады что выбрали вас в качестве дизайнеров для нашей квартиры в ЖК Рихард. У нас был основной вопрос в зале с кухней, так как площадь ее очень небольшая всего 20 кв.метров и мы не понимали как лучше все выстроить.",
  ...
  },
  {
    id: 6,
    title: "100 баллов",
    text: "Готова поставить студии просто 100 баллов! Отработали весь проект от начала и завершения авторского надзора на высшем уровне. Я осталась очень довольна результатом выполненной работы команды, которые помогли мне создать идеальный дизайн для моей квартиры.",
    subtitle: "Венера",
    info: "Москва, 64 м²",
  ...
  },
  {
    ...
    text: "Мы находимся не в Москве, поэтому для нас было важно, чтобы связь со студией была постоянной. Такой запрос был учтен. Дизайнер Дмитрий всегда был на связи, устраивал зум звонки, записывал видео с подробными объяснениями.",
    subtitle: "Анастасия",
  ...
  },
  {
    ...
    text: "Хочу выразить благодарность за отличный мини-проект двухкомнатной квартиры. Очень понравились креативность и профессионализм специалистов. А благодаря тому, что дизайнеру удалось учесть все мои пожелания — получилось создать очень комфортную планировку.",
    subtitle: "Антон",
...
function renderSlide() {
  ...
  sldr.innerHTML = `
    <div class="w-[40%] pt-4">
      <div class="text-[70px] leading-none text-white/80 font-serif">“</div>

      <h2 class=" text-[24px] font-medium text-black leading-[1.3]">
        ${item.title}
      </h2>

      <p class="mt-12 text-[18px]  tracking-[0.024rem]  text-black ">
        ${item.text}
      </p>
    </div>
    ...
    <div class="w-[60%]">
      <div class="flex items-start justify-between mb-8">
        <div>
          <p class="text-[16px] font-medium text-black leading-tight">
            ${item.subtitle}
          </p>
          <p class="mt-1 text-[14px] text-gray-500 leading-tight">
            ${item.info}
          </p>
        </div>

        <div class="text-[26px] font-medium text-black">
          ${item.id}
      ...
      <div class="group relative overflow-hidden">
        ...
        <button
          id="nextBtn"
          class="absolute right-8 top-1/2 -translate-y-1/2 w-16 h-16 rounded-full bg-[#ccfe0f] flex items-center justify-center text-[22px] text-black opacity-0 group-hover:opacity-100 transition duration-300 cursor-pointer"
        >
          <i class="fa-solid fa-arrow-right" style="color: rgb(0, 0, 0);"></i>
        </button>
      </div>
    </div>
  `;
